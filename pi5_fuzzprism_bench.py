...
print("=== INITIALIZING FUZZPRISMEDGE (RASPBERRY PI 5 ARM CPU) ===")

# ==========================================
# 1. LOAD MODELS INTO CPU RAM
# ==========================================
...
# --- CPU WARM-UP ---
# Even without CUDA, PyTorch needs a few runs to allocate memory buffers and build execution graphs
print("\n[Warming up CPU caches (10 iterations)...]")
with torch.no_grad():
    for _ in range(10):
...
def run_precision_benchmark(state_name, bat_val, mot_val, iterations=50):
    ...
    for i in range(iterations):
        ...
        # Track highest RAM jump
        current_ram = process.memory_info().rss / (1024 * 1024)
        if (current_ram - base_ram) > peak_ram_diff:
            peak_ram_diff = current_ram - base_ram
    ...
    print(f"  |-- Avg CPU AI Latency    : {avg_ai:.2f} ms")
    print(f"  |-- Total System Latency  : {(avg_fuzz + avg_ai):.2f} ms")
    print(f"  |-- Active RAM Fluctuation: {peak_ram_diff:.2f} MB")
    print("-" * 60)
